...
case $IL in
  ...
  2)echo -e "是否需要开启通过steam组输入指令功能？ \n1.开启 \n2.不开启 \n请输入数字";
    ...
    1)echo "输入你的steam64位id"
	  ...
	  read IQ
	  if [ $IQ == 1 ]; then
	    sed -i 's/QE/True/' /root/ASF/config/ASF/master.json
	  else
	    sed -i 's/QE/False/' /root/ASF/config/ASF/master.json
	  fi #Mark
	  echo -e "请选择是否挂尚可退款的游戏 \n1.是 \n2.否"
	  read FL
	  if [ $FL == 2 ]; then
	    sed -i 's/FD/False/' /root/ASF/config/ASF/master.json
	  else
	    sed -i 's/FD/True/' /root/ASF/config/ASF/master.json
	  fi
	  echo -e "请选择挂卡结束后是否退出ASF程序 \n1.是 \n2.否"
	  read SW
	  if [ $SW == 2]; then
	    sed -i 's/SD/False/' /root/ASF/config/ASF/master.json
	  else
	    sed -i 's/SD/True/' /root/ASF/config/ASF/master.json
	  fi
	  echo -e "请输入您在挂卡时想要显示的自定义内容，如要输入中文，请确认您的SSH客户端支持中文输入 \n如果不需设置，直接Enter即可跳过"
	  read CR
	  if [ -z $CR ];then
	    sed -i '/CT/d' /root/ASF/config/ASF/master.json
	  else
	    sed -i '/s/CT/$CR' /root/ASF/config/ASF/master.json
	  fi 
     ;;
    2)cd ASF/config
      touch master.json
